...
for ($i = 0; $i < count($portal_header_menus); $i++)
{
	...
	if ($portal_header_menus[$i]['view_by'] != 0 ) // assign all if not hidden
	{
		...
		else if ($portal_header_menus[$i]['view_by'] == 2 && $user->data['is_registered']) //view by registered
		{
		...
		else if ($user->data['user_type'] == 3 && $user->data['is_registered']) // admin only
		{
			$template->assign_block_vars('portal_header_menus_row', array(
				'PORTAL_HEADER_MENU_NAME' => $name,
		...
		else if ($portal_header_menus[$i]['view_by'] == 4 && $user->data['is_registered']) //view by moderators & staff
		{
			$template->assign_block_vars('portal_header_menus_row', array(
				'PORTAL_HEADER_MENU_NAME' => $name,
...
$template->assign_vars(array(
	...
	'HM_PORTAL_DEBUG'		=> sprintf($user->lang['PORTAL_DEBUG_QUERIES'], ($queries) ? $queries : '0', ($cached_queries) ? $cached_queries : '0', ($total_queries) ? $total_queries : '0'),
));
?>