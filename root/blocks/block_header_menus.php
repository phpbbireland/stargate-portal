...
*/
	...
	global $db, $user, $_SID, $_EXTRA_URL, $k_groups;
	...
	for ($i = 0; $i < count($portal_menus); $i++)
	{
		...
		if ($process_menu_item)
		{
			...
			$template->assign_block_vars('portal_header_menus_row', array(
				'EXTERN'					=> $link_option,
				'PORTAL_HEADER_MENU_NAME'	=> ($is_sub_heading) ? $name : '',
				'PORTAL_HEADER_MENU_NAME' 	=> $name,
				'PORTAL_HEADER_MENU_LINK' 	=> ($portal_menus[$i]['sub_heading']) ? '' : $link,
				'PORTAL_HEADER_MENU_ICON'	=> ($portal_menus[$i]['menu_icon']) ? '<img src="' . $phpbb_root_path . 'images/block_images/' . $portal_menus[$i]['menu_icon'] . '" height="16px" width="16px" alt="" />' : '<img src="' . $phpbb_root_path . 'images/block_images/none.gif" height="15px" width="15px" alt="" />',
