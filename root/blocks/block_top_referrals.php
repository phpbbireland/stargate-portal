...
isset($_SERVER['HTTP_REFERRALS']) ? $http_referrals = $_SERVER['HTTP_REFERRALS'] : $http_referrals = '';

if($http_referrals)						// Do we have any referrals?
{
	$http_host = $_SERVER['HTTP_HOST'];

	$int_check = strpos($http_referrals, $http_host);

	if(!$int_check)			// check if an internal referrals?
	{
