...
if(!function_exists('process_for_vars'))
{
	function process_for_vars($data)
	{
		...
		//later add to database to allow a bigger range and var to be used in other sections.. 29 March 2009
		/*
		if($in)
		{
...
if(!function_exists('sgp_build_minimod'))
{
	...
	{
		global $phpbb_root_path, $user, $template, $db, $k_config, $config;

		$queries = $cached_queries = $i = 0;
		$same_mod_count = 1;
		...
		foreach($mods as $mod)
		{
			...
			if($mod['mod_origin'])
			{
				$template->assign_block_vars('our_mod_'. $mod['mod_type'] . '_row', array(
					...
					'MOD_DETAILS'			=> htmlspecialchars_decode($mod['mod_details']),
					'MOD_THIS'				=> $i++,
